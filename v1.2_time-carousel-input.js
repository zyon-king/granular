...
function setupTimeCarousels(
    ...
    minutesCarouselId, minutesItemsId, // <-- minutesCarouselId CORRIGIDO AQUI
    ...
    // AQUI ESTAVA O ERRO: minutesMinutesId foi corrigido para minutesCarouselId
    const minutesCarousel = document.getElementById(minutesCarouselId);
    ...
    function setupMainCarouselScroll(itemsElement, total, isHourCarousel) {
        if (!itemsElement) return; // Adicionada segurança
    ...
    function populateSelectionCarousel(itemsContainer, total, parentOverlay, currentValGetter, currentValSetter, isHourCarousel) {
        if (!itemsContainer) return; // Adicionada segurança
        itemsContainer.innerHTML = '';
        for (let r = 0; r < SELECTION_NUM_REPEATS; r++) {
            for (let i = 0; i < total; i++) {
                ...
                item.addEventListener('click', (event) => {
                    ...
                    // Certifique-se de que hoursItems ou minutesItems existam antes de chamar updateMainCarouselPosition
                    if (isHourCarousel && hoursItems) {
                        updateMainCarouselPosition(hoursItems, selectedValue);
                    } else if (minutesItems) {
                        updateMainCarouselPosition(minutesItems, selectedValue);
                    }
    ...
    function setupSelectionCarouselInteraction(
        ...
        if (!mainItemsElement || !selectionOverlay || !selectionItemsContainer) return; // Adicionada segurança
        ...
        mainItemsElement.addEventListener('click', (event) => {
            ...
            const initialTransformY = -(initialTargetFullIndex * CAROUSEL_ITEM_HEIGHT) + (CAROUSEL_BUFFER_ITEMS * CAROUSEL_ITEM_HEIGHT);
        ...
        // Use selectionCarousel (a variável local correta) aqui.
        if (isHourCarousel && hoursSelectionCarousel) { // Verifica se o elemento existe e é de hora
             hoursSelectionCarousel.addEventListener('wheel', (event) => {
                event.preventDefault();
        ...
        } else if (!isHourCarousel && minutesSelectionCarousel) { // Verifica se é de minuto
             minutesSelectionCarousel.addEventListener('wheel', (event) => {
                event.preventDefault();
                let newLogicalIndex = currentValGetter();
